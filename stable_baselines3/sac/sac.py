...
from stable_baselines3.common.buffers import ReplayBuffer, DictReplayBuffer, ExtendedReplayBuffer
...
class SACMaster(SAC):
    def __init__(
        ...
        if load_subpolicies:
            ...
            policy_kwargs["trainable_subpolicy"] = trainable_subpolicy
    ...
    def _setup_model(self) -> None:
        if self.replay_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                ...
                self.replay_buffer_class = ExtendedReplayBuffer
    ...
    def _update_step(self, 
        ...
        # Get current Q-values estimates for each critic network
        # using action from the replay buffer
        current_q_values = critic(replay_data.observations, replay_data.master_actions if is_Master else replay_data.actions)
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            # Sample replay buffer
            replay_data = self.replay_buffer.sample(batch_size, env=self._vec_normalize_env)  # type: ignore[union-attr]
            self._update_step(replay_data,
                              gradient_step,
                              self.actor,
                              self.critic,
                              self.critic_target,
                              ent_coef_optimizer=self.ent_coef_optimizer,
                              log_ent_coef=self.log_ent_coef,
                            #   ent_coef_tensor=self.ent_coef_tensor,
                              ent_coef_losses=ent_coef_losses,
                              ent_coefs=ent_coefs,
                              actor_losses=actor_losses,
                              critic_losses=critic_losses,
                              is_Master=True
                              )
        # update one time all sub_plciies
        for subpolicy in self.policy.subpolicies:
            replay_data = self.replay_buffer.sample(batch_size, env=self._vec_normalize_env)
            self._update_step(
                replay_data,
                gradient_step=0,
                actor=subpolicy.actor,
                critic=subpolicy.critic,
                critic_target=subpolicy.critic_target,
                ent_coef_tensor=self.log_ent_coef if self.log_ent_coef is not None else self.ent_coef_tensor
            )
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            # sub_actions = pool_out
        else:
            # Note: when using continuous actions,
            # we assume that the policy uses tanh to scale the action
            # We use non-deterministic action in the case of SAC, for TD3, it does not matter
            assert self._last_obs is not None, "self._last_obs was not set"
            # unscaled_action, (weights, sub_actions), _ = self.predict(self._last_obs, deterministic=False)
        ...
        return action, weights, #sub_actions
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            # Select action randomly or according to policy
            # actions, buffer_actions, sub_actions = self._sample_action(learning_starts, action_noise, env.num_envs)
            actions, buffer_actions = self._sample_action(learning_starts, action_noise, env.num_envs)
            
            # Rescale and perform action
            new_obs, rewards, dones, infos = env.step(actions)
            # pprint(f"infos: {infos} \n len: {len(infos)} \n type:{type(infos)}")
            # print(f"weigths: {buffer_actions} \n type:{type(buffer_actions)}")
            
            for info, w in zip(infos, buffer_actions):
                info["weigths"] = w
            # pprint(f"infos updated: {infos}")
            ...
            # self._store_transition(replay_buffer, buffer_actions, sub_actions, new_obs, rewards, dones, infos)  # type: ignore[arg-type]
            self._store_transition(replay_buffer, buffer_actions, actions, new_obs, rewards, dones, infos)  # type: ignore[arg-type]
