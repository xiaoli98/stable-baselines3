...
class SACMaster(SAC):
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            with th.no_grad():
                weights = self.policy.get_weights(self._last_obs, deterministic=False)
    ...
    def _store_transition(
        ...
        with th.no_grad():
            pool_out = self.policy.get_pool_out(new_obs, deterministic=True)
        master_obs = th.flatten(th.as_tensor(new_obs), start_dim=1)
        flatten_pool_out = th.flatten(th.permute(pool_out, (1,0,2)), start_dim=1)
        new_obs = th.cat([master_obs, flatten_pool_out], dim=1).cpu().numpy()
        self.new_pool_out = pool_out
        ...
        for i, done in enumerate(dones):
            if done and infos[i].get("terminal_observation") is not None:
                if isinstance(next_obs, dict):
                    ...
                    # cat pool output as zeros to terminal observation, since we don't need
                    terminal_obs = infos[i]["terminal_observation"].flatten()
                    pool_shape_zeros = np.zeros((self.policy.n_actions, np.prod(self.policy.env_action_space.shape))).flatten()
                    next_obs[i] = np.concatenate((terminal_obs, pool_shape_zeros))
                    # VecNormalize normalizes the terminal observation
                    if self._vec_normalize_env is not None:
                        next_obs[i] = self._vec_normalize_env.unnormalize_obs(next_obs[i, :])
        
        # print(f"next_obs shape: {next_obs.shape}\t last_original_obs shape: {self._last_original_obs.shape}")
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            self._last_pool_out = self.new_pool_out
