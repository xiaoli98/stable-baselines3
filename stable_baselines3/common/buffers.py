...
class ExtendedReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        observation_space: spaces.Space,
        action_space: spaces.Space,
        master_action_space: spaces.Space,
        device: Union[th.device, str] = "auto",
        n_envs: int = 1,
        optimize_memory_usage: bool = False,
        handle_timeout_termination: bool = True,
        ...
        self.master_action_space = master_action_space
        self.master_action_dim = get_action_dim(master_action_space)
        self.master_actions = np.zeros(
                    (self.buffer_size, self.n_envs, self.master_action_dim), dtype=self._maybe_cast_dtype(action_space.dtype)
                )
    ...
    def add(self, 
        ...
        master_actions = master_actions.reshape((self.n_envs, self.master_action_dim))
        self.master_actions[self.pos] = np.array(master_actions)
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        data = (
            self._normalize_obs(self.observations[batch_inds, env_indices, :], env),
            self.actions[batch_inds, env_indices, :],
            self.master_actions[batch_inds, env_indices, :],
            next_obs,
            # Only use dones that are not due to timeouts
            # deactivated by default (timeouts is initialized as an array of False)
            (self.dones[batch_inds, env_indices] * (1 - self.timeouts[batch_inds, env_indices])).reshape(-1, 1),
        ...
        return ExterndedReplayBufferSamples(*tuple(map(self.to_torch, data)))
